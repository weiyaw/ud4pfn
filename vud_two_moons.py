...
idx2d = np.arange(gs * gs).reshape(gs, gs)  # row-major grid indices
...
np.savez(outdir / f"vud_two_moons_n{n}_eval{m}_est{args.n_estimators}.npz",
...
fig.savefig(outdir / f"vud_two_moons_n{n}_eval{m}_est{args.n_estimators}.png", dpi=150)
print(f"\nsaved to {outdir}")
