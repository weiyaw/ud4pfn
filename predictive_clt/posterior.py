...
def compute_gn(predictive_rule, t, x_grid, x_prev, y_prev):
    ...
    return predictive_rule.predict_event(
        t=t, x_new=x_grid, x_prev=x_prev, y_prev=y_prev
    )


def sample_gn_plus_1(key, predictive_rule, t, x_grid, x_prev, y_prev, size=100):
    ...
    x_plus_1 = x_prev[idx]
    y_plus_1, _ = predictive_rule.sample(
        key=key_sample, x_new=x_plus_1, x_prev=x_prev, y_prev=y_prev, size=1
    )
    y_plus_1 = np.asarray(y_plus_1).reshape(-1)
...
def compute_g0_to_gn(predictive_rule, t, x_grid, x_prev, y_prev):
    """
