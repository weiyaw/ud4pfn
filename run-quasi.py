...
def run_single_outer_path(rng, X0, y0, k_samp, m_inner, x_new, tag, savedir, clf):
    ...
    np.save(savedir / f"{tag}.npy", bias_vals)
    return bias_vals
...
def main(cfg: DictConfig):
    ...
    k_samp = np.unique(np.concatenate([
        np.arange(1, 21),
        np.arange(25, 101, 5),
        np.arange(120, n_horizon+1, 20)
    ])).astype(int)
