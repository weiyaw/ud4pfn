...
def fit_copula_cregression(
    ...
    start = time.time()
    vn_perm = mvcr.update_pn_loop_perm(rho_opt, rho_opt_x, y_perm, x_perm)[
        0
    ].block_until_ready()
    end = time.time()
    logging.info("Fit time: {}s".format(round(end - start, 3)))

    copula_cregression_obj = namedtuple(
        "copula_cregression_obj",
        ["vn_perm", "rho_opt", "rho_x_opt", "preq_loglik", "x_perm"],
    )
    # return copula_cregression_obj(vn_perm,rho_opt,rho_opt_x,-opt.fun,x_perm)
    return copula_cregression_obj(vn_perm, rho_opt, rho_opt_x, -fixed_opt_fun, x_perm)
...
def predict_copula_cregression(copula_cregression_obj, y_test, x_test):
    ...
    start = time.time()
    logcdf_conditionals, logpdf_joints = mvcr.update_ptest_loop_perm_av(
        copula_cregression_obj.vn_perm,
        copula_cregression_obj.rho_opt,
        copula_cregression_obj.rho_x_opt,
        copula_cregression_obj.x_perm,
        y_test.reshape(-1, 1),
        x_test,
    )
    logcdf_conditionals = logcdf_conditionals.block_until_ready()  # for accurate timing
    end = time.time()
    logging.info("Prediction time: {}s".format(round(end - start, 3)))
...
def predictive_resample_cregression(
    copula_cregression_obj,
    x,
    y_test,
    x_test,
    ...
    logcdf_conditionals_pr = (
        logcdf_conditionals_pr.block_until_ready()
    )  # for accurate timing
    end = time.time()
...
def check_convergence_pr_cregression(
    copula_cregression_obj,
    x,
    y_test,
    x_test,
    ...
    logging.info("Predictive resampling...")
    start = time.time()
    logcdf_conditionals_pr, logpdf_joints_pr, pdiff, cdiff = (
        samp_mvcr.pr_loop_conv_cregression_B(
            subkey,
            logcdf_conditionals,
            logpdf_joints,
            x,
