...
def get_metrics(rep_dirs: list[str], data, t_idx, alpha=0.05):
    ...
    gn_all_rep_t = [read_pickle(f"{outdir}/gn.pickle") for outdir in rep_dirs]
    gn_plus_1_all_rep_t = [
        read_pickle(f"{outdir}/gn_plus_1.pickle") for outdir in rep_dirs
    ]
    g0_to_gn_all_rep_t = [
        read_pickle(f"{outdir}/g0_to_gn.pickle") for outdir in rep_dirs
    ]
    ...
    for outdir in rep_dirs:
        ...
        copula_point_bands.append(
            build_bootstrap_pointwise_band(mean, cdf_samples, alpha=alpha)
        )
        copula_simul_bands.append(
            build_bootstrap_simultaneous_band(mean, cdf_samples, alpha=alpha)
        )
    ...
    out |= {
        ...
        "copula_simul_width": np.mean([b["width"] for b in copula_simul_bands])
        if copula_simul_bands
        else np.nan,
        "copula_point": compute_pointwise_coverage(true_prob, copula_point_bands),
        "copula_point_width": np.mean([b["width"] for b in copula_point_bands])
        if copula_point_bands
        else np.nan,
    }
...
def get_df(all_dirs: list[str], alpha: float = 0.05) -> pd.DataFrame:
    df = []
    for experiment_name, dirs in tqdm(group_by_setup(all_dirs).items()):
        data = read_pickle(f"{dirs[0]}/data.pickle")
        df.append(
            {
...
for n in [200, 500, 1000]:
    all_dirs = [str(path) for path in find_run_directories(id_dir, rf".+n={n} .+")]
...
print(
    dfs05[dfs05["setup"].str.contains("multivariate", na=False)]
...
print(
    dfs20[dfs20["setup"].str.contains("multivariate", na=False)]
    .reset_index(drop=True)
    .to_markdown(index=False, floatfmt=".2f")
)
# %%
