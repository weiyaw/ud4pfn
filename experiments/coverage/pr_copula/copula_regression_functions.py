...
def _logalpha_default(i, n_offset=0):
    t = i + n_offset
    return jnp.log(2.0 - (1.0 / (t + 1))) - jnp.log(t + 2)


def _logalpha_type1(i, d, n_offset=0):
    # d is the dim of feature x
    logC = np.log(10) * (1 - d) / (d + 4)
    beta = 0.5 + d / (d + 4)
    t = i + n_offset
    return logC - beta * jnp.log(t + 2)
...
def negpreq_jconditloglik_perm(hyperparam, z_perm):
    ...
    vn, logcdf_conditionals_yn, logpdf_joints_yn, preq_loglik = (
        mvcd.update_pn_loop_perm(rho, z_perm)
    )
...
def update_pn(carry, i):
    ...
    logcdf_conditionals_yn, logpdf_joints_yn = update_copula(
        logcdf_conditionals_yn, logpdf_joints_yn, u, v, logalpha_x, rho
    )
...
def update_ptest_single(carry, i):
    ...
    logcdf_conditionals_ytest, logpdf_joints_ytest = mvcd.update_copula_single(
        logcdf_conditionals_ytest, logpdf_joints_ytest, u, v, logalpha_x, rho
    )
...
update_ptest_single_loop_perm = jit(
    vmap(update_ptest_single_loop, (0, None, None, 0, None, None))
)  # vmap over vn_perm
...
def update_ptest_single_loop_perm_av(vn_perm, rho, rho_x, x_perm, y_test, x_test):
    ...
    logcdf_conditionals, logpdf_joints = update_ptest_single_loop_perm(
        vn_perm, rho, rho_x, x_perm, y_test, x_test
    )
...
update_ptest_loop_perm_av = jit(
    vmap(update_ptest_single_loop_perm_av, (None, None, None, None, 0, 0))
)
### ###
