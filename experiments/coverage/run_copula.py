...
from predictive_clt import TabPFNRegressorPPD
...
def save_copula_samples_for_rep(rep_dir: str, cfg: DictConfig) -> None:
    ...
    outpath = directory / f"copula-{cfg.rollout_times}-{cfg.rollout_length}.pickle"
    ...
    metadata = load_run_metadata(
        directory, required_fields=("setup", "n_est")
    )
    supported = {
    ...
    if metadata["setup"] not in supported:
        names = ", ".join(sorted(supported))
        raise ValueError(
            f"Copula baseline does not support setup '{metadata['setup']}'. "
            f"Supported setups: {names}"
        )
